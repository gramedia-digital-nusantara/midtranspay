...
Midtrans about a request.  Response formats can vary depending on the
type of error encountered from Midtrans, so the only two parameters
...
from midtranspay import mixins, helpers, payment_types


class ResponseBase(mixins.SerializableMixin):
    '''
    Base class for all responses from Midtrans.  The only two things
    ...
    def __init__(self, status_code, status_message, *args, **kwargs):
        '''
        :param status_code: Transaction status code supplied by Midtrans.
...
class ChargeResponseBase(ResponseBase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.transaction_time = \
            helpers.parse_midtrans_datetime(
        ...
        self.gross_amount = \
            helpers.parse_midtrans_amount(
                kwargs.get('gross_amount', None))


class CreditCardChargeResponse(ChargeResponseBase):
    '''
    Response from Midtrans, returned after a
    :py:class:`midtrans.request.ChargeRequest` with a charge type of
    :py:class:`midtrans.payment_types.CreditCard`.
    '''
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.saved_token_id_expired_at = helpers.parse_midtrans_datetime(
...
class VirtualAccountPermataChargeResponse(ChargeResponseBase):
    """
        Response from Midtrans, returned after a
        :py:class:`midtrans.request.ChargeRequest` with a charge type of
        :py:class:`midtrans.payment_types.VirtualAccountPermata`.
...
class VirtualAccountBcaChargeResponse(ChargeResponseBase):
    """
        Response from Midtrans, returned after a
        :py:class:`midtrans.request.ChargeRequest` with a charge type of
        :py:class:`midtrans.payment_types.VirtualAccountBca`.
...
class VirtualAccountBniChargeResponse(ChargeResponseBase):
    """
        Response from Midtrans, returned after a
        :py:class:`midtrans.request.ChargeRequest` with a charge type of
        :py:class:`midtrans.payment_types.VirtualAccountBni`.
...
class VirtualAccountMandiriChargeResponse(ChargeResponseBase):
    """
        Response from Midtrans, returned after a
        :py:class:`midtrans.request.ChargeRequest` with a charge type of
        :py:class:`midtrans.payment_types.VirtualAccountMandiri`.
...
def build_charge_response(request, *args, **kwargs):
    ...
    :param request: The request that was submitted to Midtrans.
        The charge_type is used to determine the appropriate
        response type to build.
    :type request: :py:class:`midtranspay.request.ChargeRequest`
    '''
...
class StatusResponse(ResponseBase):
    ...
    Returned from Midtrans after submitting a
    :py:class:`midtranspay.request.StatusRequest`
    '''
...
class CancelResponse(ResponseBase):
    '''
    Data returned from Midtrans after submitting a
    :py:class:`midtranspay.request.CancelRequest`.
    '''
...
class ApproveResponse(ResponseBase):
    '''
    Data returned from Midtrans after submitting a
    :py:class:`midtranspay.request.ApprovalRequest`
    '''
